...
const AssetDetailsPage = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 space-y-6">
      ...
      {(asset.manufacturer || asset.modelNumber || asset.assetNumber || asset.serialNo || asset.installYear || asset.filterSize || asset.beltSize || asset.site || asset.location || asset.room) && (
        <div className="bg-surface rounded-lg border border-secondary-200 shadow-sm p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {asset.room && (
              <div className="col-span-2 p-4 bg-primary-50 rounded-lg border-2 border-primary-200">
                <p className="text-xs font-medium text-primary-600 mb-2 flex items-center gap-1">
                  <MapPin className="w-4 h-4" />
                  Facility Location
                </p>
                <div className="flex items-center gap-2 text-sm">
                  <span className="font-semibold text-[#232249]">{asset.room.floor.building.name}</span>
                  <span className="text-primary-400">→</span>
                  <span className="font-medium text-slate-700">{asset.room.floor.number}</span>
                  <span className="text-primary-400">→</span>
                  <span className="font-medium text-slate-700">{asset.room.name}</span>
                </div>
              </div>
            )}
